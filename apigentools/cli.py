...
def get_cli_parser():
    ...
    p.add_argument(
        ...
        help="The language to run the specified action against. (Default: None to run all)",
    )
    p.add_argument(
        "-av", "--api-versions",
        action="append",
        default=env_or_val("APIGENTOOLS_API_VERSION", None),
        help="The API version to run the specified action against. (Default: None to run all)",
    )
