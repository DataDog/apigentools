...
@click.group()
@click.option('--verbose', default=env_or_val("APIGENTOOLS_VERBOSE", False, __type=bool), is_flag=True,
              help="Whether or not to log the generation in verbose mode")
@click.option("--git-via-https", is_flag=True, default=env_or_val("APIGENTOOLS_GIT_VIA_HTTPS", False, __type=bool),
              help="Use HTTPS for interacting with the git repositories. Otherwise use SSH."
              )
@click.option("--git-via-https-oauth-token", default=env_or_val("APIGENTOOLS_GIT_VIA_HTTPS_OAUTH_TOKEN", ""),
              help="Insert OAuth token in the repo URL when using HTTPS for interacting with the git repositories."
              )
@click.option("--git-via-https-installation-access-token",
...
@click.option("-l", "--languages", multiple=True,
...
@click.option("-av", "--api-versions", multiple=True,
