...
def test_command_get_generated_lang_dir():
    ...
    config = Config.from_dict({"languages": {"java": {"github_repo_name": "repo-name"}}})
...
def test_command_get_generated_lang_version_dir():
    args = flexmock(generated_code_dir="/some/path")
    config = Config.from_dict({"languages": {"java": {"github_repo_name": "repo-name", "version_path_template": "{{spec_version}}"}}})
    mc = MyCommand(config=config, args=args)
